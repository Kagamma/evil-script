...
- Without annotations, the JIT compiler will rely on static type checking during interpretation to determine the type of a variable, which works fine for named variables and functions, but not will work well for maps.